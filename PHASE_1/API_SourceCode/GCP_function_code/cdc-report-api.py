...
def cdc_report_api(request):
    ...
    local_log["end_point"] = request.base_url + "cdc-report-api-geo"
                        ...
                        if data.val().__contains__("reports"):
                            for report in data.val()["reports"]:
                                ...
                                if not g:
                                    ...
                                    local_log["status_code"] = 400
                                    db.child("logs").push(local_log)
                                    return "Require valid location", 400
                                else:
                                    if report.__contains__("locations"):
                                        for geo_id in report["locations"]:
                                            if g.geonames_id == geo_id["geonames_id"]:
                                                if len(key_terms) == 0:
                                                    reports.append(data.val())
                                                    report_menu.append(data.key())
                                                    i = i + 1
                                                    ...
                                                    for term in key_terms_a:
                                                        if term.lower() in data.val()["main_text"].lower():
                                                            ...
                                                            break
                                                break
                                            h = geocoder.geonames(geo_id["geonames_id"], key = uname, method = 'hierarchy')
                                            for loc in h:
                                                if loc.geonames_id == geo_id["geonames_id"]:
                                                    if len(key_terms) == 0:
                                                        reports.append(data.val())
                                                        report_menu.append(data.key())
                                                        i = i + 1
